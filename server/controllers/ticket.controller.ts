...
import { uploadAttachments, deleteAttachments } from "./asset.controller";
...
export const deleteTicket = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const filePaths = ticket.attachments?.map(att => `attachments/${att.public_id}`) || [];
      const deleteResult = await deleteAttachments(filePaths);

      if (!deleteResult.success) {
        return next(new ErrorHandler(deleteResult.error || "Failed to delete attachments", 500));
      }
